...
// 参考： https://zhuanlan.zhihu.com/p/257101455
...
pub mod pallet {
	...
	struct CoinCapData {
		...
		#[serde(deserialize_with = "de_string_to_bytes")]
		name: Vec<u8>,
		#[serde(deserialize_with = "de_string_to_price")]
		priceUsd: (u64,u64),
	}
	///string 25.781371372 to (25,7813)
	pub fn de_string_to_price<'de, D>(de: D) -> Result<(u64,u64), D::Error>
		where
			D: Deserializer<'de>,
	{
		let s: &str = Deserialize::deserialize(de)?;
		let mut sp=s.split(".");
		//Ok((TryInto::<u64>::try_into(sp.next().expect("0")).expect(0), TryInto::<u64>::try_into(sp.next().expect("0")).expect(0)))
		let integer_part =sp.next().unwrap();
		let decimal_part =sp.next().unwrap();
		let decimal_part_2 = &decimal_part[0..5];
		let a =Ok((integer_part.parse::<u64>().unwrap(), decimal_part_2.parse::<u64>().unwrap())); // &str 转换为 u64，用parse
		a
	}

	impl fmt::Debug for CoinCapInfo {
		// `fmt` converts the vector of bytes inside the struct back to string for
		//   more friendly display.
		fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
			let price=arrform!(20,"{}.{}",&self.data.priceUsd.0, &self.data.priceUsd.1);
			write!(
				f,
				"{{ id: {}, symbol: {}, name: {}, priceUsd:{} }}",
				str::from_utf8(&self.data.id).map_err(|_| fmt::Error)?,
				str::from_utf8(&self.data.symbol).map_err(|_| fmt::Error)?,
				str::from_utf8(&self.data.name).map_err(|_| fmt::Error)?,
				price.as_str(),
				//core::fmt::format!("{}.{}",&self.data.priceUsd.0, &self.data.priceUsd.1), //换个实现，这里没有format!
			)
		}
	}
	...
	pub enum Event<T: Config> {
		...
		/// a new dot price was got
		NewPrice(Option<(u64, Permill)>),
	}
	...
	impl<T: Config> Pallet<T> {
		...
		pub fn submit_price_unsigned(origin: OriginFor<T>, price: (u64, Permill)) -> DispatchResult {
			...
			Self::deposit_event(Event::NewPrice(Some(price)));
			Ok(())
		}
	...
	impl<T: Config> Pallet<T> {
		...
		fn fetch_price_info() -> Result<(), Error<T>> {
			...
			if let Ok(dot_info) = Self::fetch_coincap_parse() {
				log::info!("dot:{:?}",dot_info);
				let p_decimal =dot_info.data.priceUsd.1 as u32;
				let p_p =Permill::from_percent(p_decimal);
				let p_par =Permill::from_parts(p_decimal);
				let dot_price = (dot_info.data.priceUsd.0, p_par);

				let mul: T::BlockNumber =10000u32.into();
				let a = p_p*mul;
				let b = p_par *mul;

				log::info!("price:{:?},a:{:?},b:{:?}",dot_price,a,b );
				Self::offchain_unsigned_price_tx(dot_price)?;
			}
			Ok(())
		}

		/// Append a new dot prices to the tail of the list, removing an element from the head if reaching
		///   the bounded length.
		...
		/// Fetch from github and deserialize the JSON to a struct
		fn fetch_n_parse() -> Result<GithubInfo, Error<T>> {
			let github= HTTP_REMOTE_REQUEST;
			let resp_bytes = Self::fetch_from_remote(github).map_err(|e| {
		...
		/// Fetch from coincap to get polkdot price and deserialize the JSON to a struct
		fn fetch_coincap_parse() -> Result<CoinCapInfo, Error<T>> {
			let http_coincap_url="https://api.coincap.io/v2/assets/polkadot";
		...
		///submit a dot price unsigned
		fn offchain_unsigned_price_tx(price: (u64,Permill)) -> Result<(), Error<T>> {
			...
			SubmitTransaction::<T, Call<T>>::submit_unsigned_transaction(call.into())
				.map_err(|_| {
					log::error!("Failed in offchain_unsigned_price_tx");
					<Error<T>>::OffchainUnsignedTxError
				})
		}
