...
fn can_breed_work() {
	new_test_ext().execute_with(|| {
		...
		//检查繁殖Kitty
		assert_ok!(KittiesModule::breed(Origin::signed(ALICE), 0, 1));
		//检查事件
		System::assert_last_event(mock::Event::KittiesModule(crate::Event::KittyCreate(
			ALICE, 2,
		)));
...
#[test]
fn can_breed_failed_invalid_same_parent_index() {
	new_test_ext().execute_with(|| {
		//检查父母为同一Kitty，是否返回正确错误
		assert_noop! {
            KittiesModule::breed(Origin::signed(ALICE),1,1),
...
#[test]
fn can_breed_failed_invalid_kittyindex() {
	new_test_ext().execute_with(|| {
		//检查父母不存在时，是否返回正确错误
		assert_noop! {
            KittiesModule::breed(Origin::signed(ALICE),0,1),
